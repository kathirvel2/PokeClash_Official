    ...
    runMove(moveOrMoveName, pokemon, targetLoc, options) {
      ...
      if (move.id !== "struggle") {
        ...
        if (move.id !== pokemon.getMoveSlot(pokemon.side.lastSelectedMoveSlot)?.id) {
          ...
          this.battle.hint(
            "In Gen 1, a Pok\xE9mon that thaws out might try to use a move that doesn't match the move of the slot it last selected (switches reset to the first slot)."
          );
          this.battle.clearActiveMove(true);
          return;
        }
